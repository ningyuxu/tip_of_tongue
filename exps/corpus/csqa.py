...
def all_csqa_dataset(cfg: DictConfig) -> List[Dict]:
    cfg = merge_config(str(Path(__file__).parent), "config.yaml", cfg)
    split = cfg.get("split", "dev")
    assert split in ["train", "dev", "test"]
    file = cfg.csqa_corpus_file[split]
    corpus_file = CORPUS / "csqa" / file
    with open(corpus_file, mode='r', encoding="utf-8") as fp:
        json_list = list(fp)
    dataset = []
    for json_str in json_list:
        data = _parse_csqa_json_line(json_str)
        dataset.append(data)
    return dataset


def _parse_csqa_json_line(json_line: str) -> Dict:
